...
export function TeachingPage() {
  ...
  return <>{showEntryFlash && <div className="teaching-entry-flash" aria-hidden="true" />}<SiteHeader language={language} onToggleLanguage={() => setLanguage((current) => current === "zh" ? "en" : "zh")} variant="secondary" activePage="teaching" /><main className={`teaching-page${course ? " course-is-open" : ""}`}><LibraryHero language={language} /><Bookshelf courses={courses} language={language} onOpen={openCourse} /></main>{course && <OpenBook course={course} language={language} phase={bookPhase} onClose={closeCourse} onOpenDeck={openDeck} />}{deck && course && <PptPreview deck={deck} language={language} color={course.color} currentSlide={slideIndex} onPrevious={() => setSlideIndex((index) => Math.max(index - 1, 0))} onNext={() => setSlideIndex((index) => Math.min(index + 1, deck.slides.length - 1))} onSelect={setSlideIndex} onClose={closeDeck} />}<SiteFooter language={language} onDemoClick={() => undefined} /></>;
}
