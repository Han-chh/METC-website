...
export function ActivitiesPage() {
  ...
  return <><Atmosphere />{showEntryFlash && <div className="teaching-entry-flash" aria-hidden="true" />}<SiteHeader language={language} onToggleLanguage={() => setLanguage((current) => current === "zh" ? "en" : "zh")} variant="secondary" activePage="activities" /><main className={`activities-page${selectedAlbum ? " album-is-open" : ""}`}><GalleryHero language={language} /><AlbumExhibition language={language} onOpen={openAlbum} /></main>{selectedAlbum && <ExpandedAlbum album={selectedAlbum} language={language} phase={albumPhase} onClose={closeAlbum} onOpenPhoto={openPhoto} />}{selectedAlbum && selectedPhotoIndex !== null && <PhotoLightbox album={selectedAlbum} photoIndex={selectedPhotoIndex} language={language} onClose={closePhoto} onPrevious={previousPhoto} onNext={nextPhoto} initialFocusRef={lightboxCloseRef} />}<SiteFooter language={language} onDemoClick={() => undefined} /></>;
}
